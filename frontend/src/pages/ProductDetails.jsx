      ...
      {stars.map((star) => (
        <span
          ...
          className={`text-2xl cursor-${readOnly ? "default" : "pointer"} ${
            star <= rating ? "text-yellow-accent" : "text-light-text/50"
          }`}
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/products/${id}/reviews`,
    ...
    <div className="min-h-screen bg-dark-bg pt-20 px-6">
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 glass-effect rounded-lg p-6 shadow-glass hover-effect">
          ...
          <div>
            <h1 className="text-4xl font-bold mb-4 text-yellow-accent">
              {product.name}
            </h1>
            <p className="text-lg mb-4 text-light-text">
              {product.description}
            </p>
            <p className="text-3xl font-semibold mb-4 text-yellow-accent">
              ${product.price}
            </p>
            ...
            <div className="mb-4 flex items-center">
              ...
              <span className="ml-2 text-light-text">
                ({averageRating} / 5)
              </span>
            </div>
          ...
          {reviews.length > 0 ? (
            <div className="space-y-4">
              {reviews.map((rev, index) => (
                <div
                  key={index}
                  className="border-b border-light-yellow/20 pb-4"
                >
                  <div className="flex items-center">
                    <p className="text-light-text font-semibold mr-2">
                      {rev.user}
                    </p>
              ...
              {relatedProducts.map((related) => (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-yellow-accent font-bold">
                      ${related.price}
                    </p>
                  </div>
                </Link>
