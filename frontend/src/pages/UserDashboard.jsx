...
export default function UserDashboard() {
  ...
  const [profileData, setProfileData] = useState({ name: '', gender: '', age: '' });
  const [passwordData, setPasswordData] = useState({ oldPassword: '', newPassword: '', confirmPassword: '' });
  const [newAddress, setNewAddress] = useState({ addressLine1: '', addressLine2: '', city: '', state: '', postalCode: '', country: '' });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setProfileData({ name: userRes.data.name || '', gender: userRes.data.gender || '', age: userRes.data.age || '' });
  ...
  const handleProfileUpdate = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put('http://localhost:5000/api/auth/me', profileData, { headers: { Authorization: `Bearer ${token}` } });
      setUser(res.data);
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      const res = await axios.put('http://localhost:5000/api/auth/change-password', { oldPassword: passwordData.oldPassword, newPassword: passwordData.newPassword }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleAddAddress = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put('http://localhost:5000/api/auth/me', { address: newAddress }, { headers: { Authorization: `Bearer ${token}` } });
      setUser(res.data);
  ...
  const handleUpdateMobile = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put('http://localhost:5000/api/auth/me', { phoneNumber }, { headers: { Authorization: `Bearer ${token}` } });
      setUser(res.data);
  ...
  const handleProfilePictureUpload = async (e) => {
    ...
    try {
      const res = await axios.put('http://localhost:5000/api/auth/me/profile-picture', formData, { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' } });
  ...
  const handleCancelOrder = async (orderId) => {
    try {
      const res = await axios.put(`http://localhost:5000/api/orders/cancel/${orderId}`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div className="flex min-h-screen bg-dark-bg">
      ...
      <div className="flex-1 p-6 ml-64 min-h-screen pt-20">
        <h1 className="text-4xl font-bold text-light-text mb-8 text-center">User Dashboard</h1>

        {activeSection === 'userDetails' && user && (
          <section className="glass-effect p-6 rounded-lg shadow-glass hover-effect">
            <h2 className="text-2xl font-semibold text-yellow-accent mb-4">User Details</h2>
            <div className="space-y-2 text-light-text">
              {user.profilePicture && (
                <img src={`http://localhost:5000${user.profilePicture}`} alt="Profile" className="w-24 h-24 rounded-full mb-4 border-2 border-yellow-accent" />
              )}
        ...
        {activeSection === 'editProfile' && user && (
          <section className="glass-effect p-6 rounded-lg shadow-glass hover-effect">
            ...
            <form onSubmit={handleProfileUpdate} className="space-y-4 max-w-lg mx-auto">
              ...
              >
                <option value="" className="text-dark-bg">Select Gender</option>
                <option value="male" className="text-dark-bg">Male</option>
                <option value="female" className="text-dark-bg">Female</option>
                <option value="other" className="text-dark-bg">Other</option>
              </select>
              <input
                type="number"
                placeholder="Age"
                ...
                min="1"
              />
              <button type="submit" className="w-full bg-yellow-accent text-dark-bg p-3 rounded hover:bg-light-yellow transition-colors hover-effect">
                Update Profile
              </button>
            </form>
            <form onSubmit={handleProfilePictureUpload} className="mt-6 space-y-4 max-w-lg mx-auto">
              <input
                type="file"
                accept="image/*"
              ...
              />
              <button type="submit" className="w-full bg-yellow-accent text-dark-bg p-3 rounded hover:bg-light-yellow transition-colors hover-effect" disabled={!profilePicture}>
          ...
          <section className="glass-effect p-6 rounded-lg shadow-glass hover-effect">
            ...
            {user.address && Object.keys(user.address).length > 0 ? (
              <div className="space-y-2 text-light-text">
                ...
                <p>{user.address.city}{user.address.city ? ', ' : ''}{user.address.state} {user.address.postalCode}</p>
        ...
        {activeSection === 'addAddress' && (
          <section className="glass-effect p-6 rounded-lg shadow-glass hover-effect">
            ...
            <form onSubmit={handleAddAddress} className="space-y-4 max-w-lg mx-auto">
              <input
        ...
        {activeSection === 'updateMobile' && (
          <section className="glass-effect p-6 rounded-lg shadow-glass hover-effect">
            ...
            <form onSubmit={handleUpdateMobile} className="space-y-4 max-w-lg mx-auto">
              <input
                type="tel"
          ...
          <section className="glass-effect p-6 rounded-lg shadow-glass hover-effect">
            ...
            {orders.length === 0 ? (
              ...
              <div className="space-y-6">
                {orders.map(order => (
                  <div key={order._id} className="glass-effect p-4 rounded-lg shadow-glass hover-effect">
